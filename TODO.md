...
| Durable candidate sizing (`shapescan` analog) | `tools/pairscan` ranks shapes and `tools/defuse` profiles distances, but **nothing in this repo counts a specific TODO candidate with the rewrite's own operand, range and encodability conditions applied** -- which is the job armlint gave `shapescan.py`, and `addpairscan.py` besides when one mask could not split a family honestly. The 2026-09 port work is the argument: four candidates, four wrong first figures, each measured with throwaway awk. The CAS loop went 614 to 19, the vector fold 9,279 to 583 and the scalar siblings 470 to 12, all three overstated because conditions went unapplied; the zero-store merge counted pairs where the unit is runs, understating the opportunity instead. A fifth estimate ran *high* (16 and 13 against a realized 10 and 2) because its read-detection was a regular expression where the check uses `inst_reads_reg64`. Every one would have been caught by a tool living in the repo. Note what does **not** port: `--selftest`'s mask verification, which exists because armlint hand-writes encoding masks and is where its own counting bugs lived. XED removes that failure mode and leaves the candidate-sizing half, which is the half x86lint lacks. Report the shape and the conditioned count side by side, as addpairscan does with `split 2->2 (encodable)` against `(unconditioned)`, so a superseded figure stays explainable rather than looking like a disagreement |
